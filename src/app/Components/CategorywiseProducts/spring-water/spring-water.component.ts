...
@Component({
  selector: 'app-spring-water',
  templateUrl: './spring-water.component.html',
  styleUrl: './spring-water.component.css'
})
export class SpringWaterComponent {
  products: any[] = [];
  ...
  ngOnInit(): void {
    ...
    this.productsService.getByCategoryId().subscribe(
      data => {
        this.Category = data;
        const distilledWaterCategoryIds = this.Category.filter(category => category.category_name === 'Spring water').map(category => category.id);
