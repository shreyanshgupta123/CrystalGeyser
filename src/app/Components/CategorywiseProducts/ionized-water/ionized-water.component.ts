...
@Component({
  selector: 'app-ionized-water',
  templateUrl: './ionized-water.component.html',
  styleUrl: './ionized-water.component.css'
})
export class IonizedWaterComponent {
  products: any[] = [];
  ...
  ngOnInit(): void {
    ...
    this.productsService.getByCategoryId().subscribe(
      data => {
        this.Category = data;
        const distilledWaterCategoryIds = this.Category.filter(category => category.category_name === 'Ionized water').map(category => category.id);
